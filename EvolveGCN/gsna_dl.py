...
class gsna_dataset():
    def __init__(self,args):
        assert args.task in ['link_pred'], 'gsna only implements link_pred'
        self.ecols = u.Namespace({'FromNodeId': 0,
                                  'ToNodeId': 1,
                                  'Weight': 2,
                                  'TimeStep': 3
                                })
        args.args = u.Namespace(args.args)

        #build edge data structure
        edges = self.load_edges(args.args)
        timesteps = u.aggregate_by_time(edges[:,self.ecols.TimeStep], args.args.aggr_time)
        ...
        self.feats_per_node = args.args.feats_per_node
    ...
    def load_edges(self,args, starting_line = 1):
